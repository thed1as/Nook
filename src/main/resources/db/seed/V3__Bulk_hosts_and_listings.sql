...
inserted_locations AS (
    INSERT INTO location (location_id, address, city, country)
        SELECT location_id, address, city, country FROM location_data
        RETURNING location_id
)

INSERT INTO listing (listing_id, created_at, description, price_per_night, average_rating, reviews_count, title, updated_at, location_id, user_id)
SELECT
...
FROM generate_series(1, 3000) AS s(i)
         ...
         JOIN location_data l ON l.rn = (mod(s.i, 1000) + 1);